...
def constructPubKey(connection,msg_hexlist):
    ...
    data, sw1, sw2 = connection.transmit([CLA,CREATE_CARD_READER_PUBKEY,P1,P2,Lc]+list(message_hexlist))
    return data,hex(sw1),hex(sw2)
...
def getCardKey(connection):
    Lc = 0
    response, sw1, sw2 = connection.transmit([CLA, GET_CARD_PUBKEY, P1, P2, Lc])
    return response, hex(sw1), hex(sw2)
...
def receiveSigned(connection):
    Lc = 0
    response, sw1, sw2 = connection.transmit([CLA, RECEIVE_SIGNED_MSG, P1, P2, Lc])
    return response, hex(sw1), hex(sw2)
...
def main():
    ...
    data, sw1, sw2 = connection.transmit([CLA, SEND_SIGNED_MSG, 0x00, 0x00, len(signedMsg)] + list(signedMsg))
    if sw1 != 0x90 or  sw2 != 0x00:
    ...
    data, sw1, sw2 = connection.transmit([CLA, RECEIVE_SIGNED_MSG, 0x00, 0x00, 0x00])
    message = data[:-128]
    signature = data[-128:]
